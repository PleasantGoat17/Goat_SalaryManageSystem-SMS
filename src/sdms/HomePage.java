...
public class HomePage { //主页类
	...
	public static void welcomePage() { //欢迎界面
		...
		lb1.setFont(new Font("方正大标宋_GBK",0,35));
		lb1.setBounds(200,30,150,100);
		lb2.setFont(new Font("方正大标宋_GBK",0,35));
		lb2.setBounds(128,100,300,80);
		bt1.setFont(new Font("方正大标宋_GBK",0,22));
		bt1.setBounds(70,210,170,70);
		bt1.setCursor(new Cursor(Cursor.HAND_CURSOR));
		bt2.setFont(new Font("方正大标宋_GBK",0,22));
		bt2.setBounds(310,210,170,70);
		bt2.setCursor(new Cursor(Cursor.HAND_CURSOR));
		
		// JButton bt_test_1=new JButton("初始化数据库");
		// bt_test_1.setFont(new Font("方正大标宋_GBK",0,12));
		// bt_test_1.setBounds(5,330,110,30);
		// bt_test_1.setCursor(new Cursor(Cursor.HAND_CURSOR));
		// JButton bt_test_2=new JButton("装填数据");
		// bt_test_2.setFont(new Font("方正大标宋_GBK",0,12));
		// bt_test_2.setBounds(420,330,90,30);
		// bt_test_2.setCursor(new Cursor(Cursor.HAND_CURSOR));
		...
		// bt_test_1.addActionListener(new ActionListener() { //进入管理员登录界面
		// 	public void actionPerformed(ActionEvent e) {
		// 		String sql = readSqlFile("lib/gongzi_copy.sql");
		// 		try (PreparedStatement ps = HomePage.connection.prepareStatement(sql)) {
		// 		} catch (SQLException e1) {
		// 			e1.printStackTrace();
		// 		}
		// 	}
		// });
		// bt_test_2.addActionListener(new ActionListener() { //进入管理员登录界面
		// 	public void actionPerformed(ActionEvent e) {
		// 		String sql = readSqlFile("lib/init_copy.sql");
		// 		try (PreparedStatement ps = HomePage.connection.prepareStatement(sql)) {
	...
	public static void loginPage() { //登录界面
		...
		lb2.setFont(new Font("方正大标宋_GBK",0,25));
		lb2.setBounds(100,100,80,100);
		lb3.setFont(new Font("方正大标宋_GBK",0,25));
		lb3.setBounds(100,150,80,100);
		tf.setFont(new Font("方正大标宋_GBK",0,25));
		tf.setBounds(170,130,230,40);
		pf.setFont(new Font(null,0,25));
		pf.setBounds(170,180,230,40);
		bt2.setFont(new Font("方正大标宋_GBK",0,25));
		bt2.setBounds(200,250,150,60);
		bt2.setCursor(new Cursor(Cursor.HAND_CURSOR));
		bt1.setFont(new Font("方正大标宋_GBK",0,17));
...
//             sql.append(line).append(";"); // 假设每个SQL语句后都有分号  
//             sql.append("\n"); // 可选，用于保持格式  
//         }  
//     } catch (IOException e) {  
//         e.printStackTrace();  
